...
class MetricViewModel  @Inject constructor(private val metricRepository: MetricRepository): ViewModel() {
    ...
    val pulse = metricRepository.getMeasuredValuesFlow(pulseDef, null).stateIn(viewModelScope, SharingStarted.Eagerly, emptyList())

    val metrics: Map<MetricDef, StateFlow<List<MeasuredValue>>> = MetricDefRegistry.defs.associateWith { def ->
        metricRepository.getMeasuredValuesFlow(def, null).stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList() )
    }
}